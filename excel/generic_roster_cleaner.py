...
positions, this engine:
  ...
  5. Updates date row to target month (fixes cleaning templates where
     roster_shortfall leaves old template dates).
  6. Trims oversized conditional-formatting sqrefs that extend past
...
def _update_dates(ws, date_zone: Dict[str, Any], year: int, month_num: int) -> None:
    """Update the date row in the worksheet to the target month.

    Uses date_zone info from detect_regions (date_row, col_start, col_end).
    Writes actual date values to the first date cell and formula =prev+1
    to subsequent cells, matching the template's existing pattern.
    """
    date_row = date_zone.get("date_row")
    col_start = date_zone.get("col_start", 9)
    col_end = date_zone.get("col_end", 36)
    if not date_row:
        return
    ...
    for d in range(1, days_in_month + 1):
        ...
        if isinstance(cell, MergedCell):
            continue
        if d == 1:
            cell.value = date(year, month_num, d)
        else:
            prev_col = col - 1
            prev_col_letter = _col_letter(prev_col)
            cell.value = f"={prev_col_letter}{date_row}+1"

    # Clear excess date cells
...
def _fill_segment(ws, *, seg_tpl: Dict[str, Any], rendered_rows: List[RenderedRow],
    ...
    for i, rr in enumerate(rendered_rows[:slot_count]):
        # Use sequential rank_seq (A1, A2, ...) instead of rr.rank_seq
        # which may be duplicated across segments or have gaps.
        fill_data_row(
            ws, data_rows[i], person=_row_to_person_dict(rr),
            seq_label=f"{seq_prefix}{i+1}",
...
def run(wb: Workbook, context: Dict[str, Any]) -> None:
    ...
    # Resolve year/month for date update
    try:
        y, m = month.split("-")
    ...
    for sheet_name, shift in sheet_shift_pairs:
        ...
        # Update date row to target month
        date_zone = regions.get("date_zone") or {}
        _update_dates(ws, date_zone, year, month_num)
